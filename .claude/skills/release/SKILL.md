...
name: release
description: |
  Create software releases with version bumping, changelog updates, git tags, and GitHub releases.
  Use when: "create a release", "bump version", "tag release", "publish version", "/release".
  Handles VERSION files, package.json, Cargo.toml, pyproject.toml, and go.mod projects.
triggers:
  - "release"
  - "create release"
  - "bump version"
...
## IMPORTANT: Use the release script

If the project has a `blueman` git remote (dual-repo setup), **always** run the automated script:

```bash
~/.claude/scripts/release.sh [patch|minor|major|vX.Y.Z] [--dry-run] [--skip-tests] [--skip-gh]
```

**Default behavior:** Pass user arguments directly. If no bump type given, defaults to `patch`.

**Examples:**
- `/release` → `~/.claude/scripts/release.sh`
- `/release minor` → `~/.claude/scripts/release.sh minor`
- `/release v2.0.0 --skip-tests` → `~/.claude/scripts/release.sh v2.0.0 --skip-tests`
- `/release --dry-run` → `~/.claude/scripts/release.sh --dry-run`

The script handles: version bump, tests, commit, tag, push to both remotes (with `gh auth switch`), and GitHub releases on both repos.

**Only fall through to the manual workflow below if the script doesn't exist or the project has no `blueman` remote.**
...
## Dual-Repo Releases (Automated Script)

For projects with dual remotes (e.g., `origin` + `blueman`), use the release script
instead of running the skill manually:
...
1. Pre-flight checks (clean tree, on main, detect project + version)
2. Version bump in all detected files (pyproject.toml, `__init__.py`, etc.)
3. Test suite (`uv run pytest`, `npm test`, etc.) — skip with `--skip-tests`
4. Commit: `chore(release): vX.Y.Z` with changelog
5. Annotated tag: `vX.Y.Z`
6. Push to origin (commits + tag)
7. `gh auth switch` to blueman user → push to blueman → switch back
8. Create GitHub release on origin repo
9. `gh auth switch` to blueman user → create release on blueman → switch back

### Dual-repo auto-detection

The script detects dual-repo mode when a `blueman` remote exists. Auth users are
extracted from remote URLs (`harrison_adobe` from SSH origin, `blueman82` from
HTTPS blueman).

### Examples

```bash
# Dry run — see all steps without executing
~/.claude/scripts/release.sh --dry-run

# Patch release (default)
~/.claude/scripts/release.sh

# Minor release, skip tests
~/.claude/scripts/release.sh minor --skip-tests

# Explicit version
~/.claude/scripts/release.sh v1.0.0
```
