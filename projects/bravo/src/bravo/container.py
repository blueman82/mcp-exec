...
def create_container(settings: Settings) -> ServiceRegistry:
    ...
    if settings.pat_encryption_key:
        registry.register(
            DependencySpec(
                name="slack_service",
                factory=lambda jira_client, pat_service: _async(
                    SlackService(settings.slack, jira_client, pat_service)
                ),
                depends_on=["jira_client", "pat_service"],
            )
        )
    else:
        registry.register(
    ...
    registry.register(
        DependencySpec(
            name="poller_service",
            factory=lambda jira_client, nudge_service, slack_service: _async(
                PollerService(settings, jira_client, nudge_service, slack_service)
            ),
            depends_on=["jira_client", "nudge_service", "slack_service"],
        )
    )
    registry.register(
        DependencySpec(
