        ...
        try:
            ...
            for channel_id, channel_data in all_channels.items():
                ...
                if jira_report_status == "PROCESSED":
                    logger.info(f"Channel {channel_id} already processed, skipping permanently")
                    continue
                ...
                if self.msg_ops:
                    ...
                    if (
                        last_activity_hours is not None
                        and last_activity_hours < self.lookback_hours
                    ):
                        logger.info(
            ...
            eligible_channels.sort(key=lambda ch: -ch.get("archived_at", 0))  # Newest first
    ...
    async def _get_hours_since_last_activity(self, channel_id: str) -> Optional[float]:
        ...
        try:
            ...
            if USE_PIPELINE_PROCESSING:
                ...
                messages = await self.msg_ops.fetch_channel_messages(channel_id=channel_id, limit=1)
            ...
            if latest_ts:
                ...
                logger.info(f"Channel {channel_id} last activity: {hours_since:.1f} hours ago")
                return hours_since
            else:
                # If no timestamp available, assume it's been quiet for a long time
                logger.warning(f"No timestamp available for channel {channel_id}, assuming quiet")
                return 999999.0
