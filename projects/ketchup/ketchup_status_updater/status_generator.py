"""
status_generator.py
This module contains the AutoStatusGenerator class, which is responsible for generating and posting status reports to Slack.
"""
...
class AutoStatusGenerator:
    ...
    async def check_for_activity(
        ...
        try:
            ...
            logger.info(f"[{channel_id}] Checking for activity since message_ts: {last_message_ts}, thread_ts: {last_thread_ts}")
            ...
            try:
                prepared_messages, metadata = await message_preparer.prepare_messages_for_auto_status(
                ...
                logger.info(f"[{channel_id}] Activity check complete: has_new_messages={has_new_messages}, has_thread_activity={has_thread_activity}, latest_ts={latest_message_ts}")
                ...
                if last_message_ts == "0":
                    ...
                    logger.info(f"First run detected for {channel_id}, keeping activity classification from metadata")
            ...
            return {
                ...
                "has_thread_activity": False,
                "has_jira_updates": False,
                "latest_message_ts": channel_config.get("auto_status_last_message_ts", "0"),  # Return current timestamp on error
        ...
        try:
            ...
            try:
                prepared_messages, channel_metadata = await message_preparer.prepare_messages_for_auto_status(
                    channel_id=channel_id,
                    since_ts=None if is_first_run else original_last_message_ts  # Use ORIGINAL timestamp, not potentially updated one
                    # Bot filtering happens automatically in channel_msg_ops
                )
            except Exception as e:
            ...
            if previous_status and not is_first_run:
                ...
                if last_run_ts:
                    last_run_date = datetime.fromtimestamp(float(last_run_ts)).strftime("%Y-%m-%d %H:%M UTC")
            ...
            logger.info(f"[MESSAGE FORMAT DEBUG] Calling _format_final_message with:")
            ...
            post_result = await self._post_to_slack_public(channel_id, final_message, status_update_id)
    ...
    def _format_final_message(self, content: str, channel_name: str, channel_id: str,
                             has_slack_activity: bool = False, has_thread_activity: bool = False,
                             has_jira_activity: bool = False) -> str:
        """Format the final message with header including why this update was triggered."""
        current_time = datetime.now(timezone.utc)
        timestamp = current_time.strftime("%d-%b-%Y, %H:%M UTC")
    ...
    async def _post_to_slack_public(self, channel_id: str, content: str, status_update_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"  - Will add button: {trust_global_enabled and (trust_global_all or trust_channel_enabled)}")
            ...
            if response.get("ok"):
                ...
                logger.info(f"Successfully posted new auto-status to {channel_id} with trust button")
    ...
    async def _store_post_timestamp(self, channel_id: str, message_ts: str) -> None:
        ...
        try:
            await self.channel_operations.update_channel_fields(
                channel_id=channel_id,
                updates={"auto_status_last_post_ts": message_ts}
            )
    ...
    async def _verify_real_activity(
        ...
        try:
            ...
            last_message_ts = original_last_message_ts or channel_config.get("auto_status_last_message_ts", "0")
            last_thread_ts = original_last_thread_ts or channel_config.get("auto_status_last_thread_ts", "0") 
            last_jira_ts = original_last_jira_ts or channel_config.get("auto_status_last_jira_comment_ts", "0")
            ...
            if last_message_ts == "0" and (last_run > 0 or last_thread_ts != "0"):
                logger.warning(f"Channel {channel_id}: Suspicious first-run detection with existing timestamps")
                return False
            ...
            try:
                ...
                if last_message_ts and last_message_ts != "0":
                    try:
                        ...
                        logger.debug(f"Applied 5-second buffer to final verification: {last_message_ts} -> {oldest_ts_buffered}")
                    except (ValueError, TypeError):
                        # If conversion fails, use original value
                        oldest_ts_buffered = last_message_ts
                        logger.warning(f"Could not apply buffer to final verification timestamp {last_message_ts}, using original")
                ...
                if response_data.get("ok"):
                    ...
                    if user_messages:
                        ...
                        logger.info(f"Channel {channel_id}: Found {len(user_messages)} real user messages")
                    else:
                        logger.info(f"Channel {channel_id}: No real user messages found in verification")
                        
                else:
                    logger.error(f"Channel {channel_id}: API error in final verification: {response_data.get('error')}")
            ...
            try:
                ...
                if hasattr(self.channel_msg_ops.check_recent_thread_activity, '__call__'):
                    # Check if it's async or sync
                    try:
                        thread_activity, _, _ = await self.channel_msg_ops.check_recent_thread_activity(
                            channel_id, last_thread_ts
                        )
                    except TypeError:
            ...
            if not has_real_message_activity and not has_real_thread_activity and not has_real_jira_activity:
                logger.info(f"Channel {channel_id}: No verified activity found in final check")
                return False
                
            logger.info(f"Channel {channel_id}: Final verification confirmed real activity (messages: {has_real_message_activity}, threads: {has_real_thread_activity}, JIRA: {has_real_jira_activity})")
            return True
