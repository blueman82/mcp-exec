...
/**
 * Creates a transition request body
...
export async function transitionJiraStatus(params: TransitionJiraStatusRequest): Promise<{ success: boolean; message: string; data?: unknown }> {
  ...
  try {
    // Create the transition request body
    const body = createTransitionRequestBody(transitionId, comment, resolution, fields);
    ...
    return formatResponse(
      true, 
      `Successfully transitioned status of issue ${issueIdOrKey}`,
      response
    );
  } catch (error) {
