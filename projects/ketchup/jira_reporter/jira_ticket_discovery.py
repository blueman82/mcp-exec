...
EXIGENCE_URL_TEMPLATE = "https://adobe.app.exigence.io/secure/index.html#/events/{event_id}/situationroom"
...
class JiraTicketDiscovery:
            ...
            if issues and len(issues) > 0:
                ...
                for issue in issues:
                    if issue.get("key", "").startswith("CSOPM-"):
                        ...
                        logger.info(f"Found JIRA ticket {ticket_id} via extended search (CSOPM preferred)")
                        return ticket_id
    ...
    async def discover_jira_ticket(
        ...
        if ticket_id:
            logger.info(f"Successfully discovered JIRA ticket {ticket_id} for channel {channel_name}")
        else:
            ...
            if self.mcp_client:
                ...
                if issues and len(issues) > 0:
                    ...
                    if ticket_id and ticket_id.startswith("CSOPM-"):
                        ...
                        logger.warning(f"Found non-CSOPM ticket {ticket_id} in CSOPM project search")
                        
            logger.info(f"No CSOPM ticket found for channel {channel_name} with exigence URL {exigence_url}")
            return None
