...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Log startup summary
        logger.info(
            "✓ Ketchup service started successfully - initialization completed in %.2fs",
            init_duration,
        )
...
async def handle_slack_event(request: Request, background_tasks: BackgroundTasks):
    ...
    if not verify_slack_signature(body, timestamp, signature):
        logger.warning("Invalid Slack signature for events endpoint")
...
async def handle_slack_command(request: Request, background_tasks: BackgroundTasks):
    ...
    # Verify Slack signature
    if not verify_slack_signature(body, timestamp, signature):
        logger.warning("Invalid Slack signature for commands endpoint")
        raise HTTPException(status_code=403, detail="Invalid signature")
...
async def readiness_check():
    ...
    return JSONResponse(
        content={"status": "ready", "service": "ketchup-app", "all_systems_operational": True}
    )
...
async def get_detailed_metrics():
    ...
    try:
        ...
        metrics_response = {"service": "ketchup-app", "timestamp": time.time(), "status": "healthy"}
        ...
        if mcp_client:
            try:
                ...
                metrics_response["mcp_health"] = {"status": "error", "error": str(e)}
        else:
        ...
        return JSONResponse(
            status_code=500, content={"error": f"Failed to retrieve metrics: {str(e)}"}
        )
