...
class TestMethodVisibility:
    ...
    async def test_modal_orchestrator_method_visibility(self, handler):
        ...
        assert hasattr(
            modal, "_create_command_feedback_modal_view"
        ), "Missing _create_command_feedback_modal_view"
        ...
        assert not hasattr(
            modal, "validate_trigger_id"
        ), "Public validate_trigger_id should not exist"
        assert not hasattr(
    ...
    async def test_status_processor_calls_private_methods(self):
        ...
        if "modal_orchestrator.validate_trigger_id" in source:
            pytest.fail(
                "StatusFlagProcessor calls public validate_trigger_id (should be _validate_trigger_id)"
            )
        if "modal_orchestrator.display_modal_via_api" in source:
            pytest.fail(
                "StatusFlagProcessor calls public display_modal_via_api (should be _display_modal_via_api)"
            )
    ...
    async def test_admin_processor_method_visibility(self, handler):
        ...
        assert hasattr(
            admin, "handle_reply_button_click"
        ), "Missing public handle_reply_button_click"
    ...
    async def test_cross_module_private_method_calls(self, handler):
        ...
        with patch("aiohttp.ClientSession") as mock_session:
            ...
            payload = {
                ...
                "channel": {"id": "D123"},
                "actions": [
                    {"action_id": "flag_status_review", "value": "D123|1234567890.123456|cmd_123"}
                ],
            }
