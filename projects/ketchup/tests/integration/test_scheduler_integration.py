...
class TestSchedulerInstantiation:
    ...
    @patch('ketchup_maintenance_fetcher.scheduler.fetch_and_store_maintenance_data', new_callable=AsyncMock)
...
class TestAllSchedulersInheritFromBaseScheduler:
    ...
    def test_all_schedulers_have_required_methods(self):
        ...
        required_methods = ['start', 'run_task', 'get_sleep_seconds',
                           '_signal_handler', '_update_health_status',
                           '_update_last_run', '_execute_task_with_tracking']
    ...
    def test_all_schedulers_have_required_attributes(self):
        ...
        required_attrs = ['running', 'health_file', 'last_run_file',
                         'interval_minutes', 'run_on_start', 'scheduler_name', 'logger']
        ...
        with patch('ketchup_status_updater.scheduler.run_auto_status', new_callable=AsyncMock):
            with patch('channel_metadata_updater.scheduler.run_metadata_update', new_callable=AsyncMock):
                with patch('ketchup_maintenance_fetcher.scheduler.fetch_and_store_maintenance_data', new_callable=AsyncMock):
                    schedulers = [
                        StatusUpdaterScheduler(),
                        MetadataUpdaterScheduler(),
                        MaintenanceFetcherScheduler(),
                    ...
                    for scheduler in schedulers:
                        for attr in required_attrs:
                            assert hasattr(scheduler, attr), f"{scheduler.__class__.__name__} missing attr {attr}"
