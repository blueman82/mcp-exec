...
class TestIncrementMonthlyCounter:
    ...
    async def test_increment_auto_status_posts(self, db_store, mock_client):
        ...
        result = await db_store.increment_monthly_counter("auto_status_posts", "2025_10", 1)
        ...
        assert call_args[1]["key"] == {"PK": {"S": "METRICS_SUMMARY"}, "SK": {"S": "AGGREGATES"}}
        assert call_args[1]["update_expression"] == "ADD #field :inc"
        assert call_args[1]["expression_attribute_names"] == {"#field": "auto_status_posts_2025_10"}
        assert call_args[1]["expression_attribute_values"] == {":inc": {"N": "1"}}

    @pytest.mark.asyncio
    async def test_increment_war_room_sent(self, db_store, mock_client):
        """Test incrementing war_room_sent counter."""
        result = await db_store.increment_monthly_counter("war_room_sent", "2025_09", 5)

        assert result is True

        # Verify field name and increment value
        call_args = mock_client.update_item.call_args
        assert call_args[1]["expression_attribute_names"] == {"#field": "war_room_sent_2025_09"}
        assert call_args[1]["expression_attribute_values"] == {":inc": {"N": "5"}}

    @pytest.mark.asyncio
    async def test_increment_default_value(self, db_store, mock_client):
        """Test default increment value is 1."""
        result = await db_store.increment_monthly_counter("auto_status_posts", "2025_10")
