...
class TestQueryCommandTargetChannelFix:
    ...
    @pytest.mark.skip(reason="CommandRouter implementation details have changed")
    @pytest.mark.asyncio
    async def test_command_router_query_command_uses_target_channel_id(self):
        """Test that CommandRouter correctly passes target_channel_id to query handler."""
        # Create mock dependencies
        mock_query_handler = AsyncMock()
        mock_query_handler.process_query_request = AsyncMock(
            return_value={"text": "Query response"}
        )
        ...
        router = CommandRouter(
            command_parser=Mock(),
            slack_user_ops=Mock(),
            short_long_handler=Mock(),
            status_handler=Mock(),
            query_handler=mock_query_handler,
            list_handler=Mock(),
            archive_handler=Mock(),
            feature_handler=Mock(),
            access_handler=Mock(),
        )

        # Create QueryCommandParams with target_channel_id
        params = QueryCommandParams(
            user_id="U12345",
            user_name="testuser",
            channel_id="D67890",  # DM channel
            ...
            context=CommandContext.DIRECT_MESSAGE,
            query_text="what's happening?",
            target_channel_id="C123456",  # Target channel
        )

        # Mock user info
        with patch.object(router.slack_user_ops, "get_user_by_id") as mock_get_user:
            mock_get_user.return_value = {"real_name": "Test User"}

            # Process the command
            result = await router._process_with_handler(
                params=params,
                user_id="U12345",
                incoming_channel="D67890",
                response_url="https://hooks.slack.com/response",
                handler=mock_query_handler,
            )

        # Verify the handler was called with correct parameters
        mock_query_handler.process_query_request.assert_called_once()
    ...
    @pytest.mark.skip(
        reason="validate_and_get_channel_info method doesn't exist in current implementation"
    )
    @pytest.mark.asyncio
    async def test_query_handler_uses_target_channel_id_from_params(self):
        """Test that SlackQueryHandler correctly uses target_channel_id from params."""
        # Create mock dependencies
        mock_channel_info_ops = Mock()
        mock_archive_ops = Mock()
        mock_openai_handler = AsyncMock()
        mock_block_kit_builder = Mock()
        mock_channel_message_ops = Mock()
        mock_slack_posting_handler = Mock()
        ...
        handler = SlackQueryHandler(
            ...
            block_kit_builder=mock_block_kit_builder,
            channel_message_ops=mock_channel_message_ops,
        ...
        # Create params with target_channel_id
        params = QueryCommandParams(
            user_id="U12345",
            user_name="testuser",
            channel_id="D67890",  # DM channel
        ...
        # Mock the resolve method to return the same channel
        with patch.object(handler, "_resolve_channel_parameter") as mock_resolve:
            ...
            with patch.object(handler, "validate_and_get_channel_info") as mock_validate:
                ...
                with patch.object(handler, "_process_channel_query") as mock_process:
                    mock_process.return_value = "Query result"

                    # Call the method
                    await handler.process_query_request(
                        params=params,
                        user_id="U12345",
                        channel_id="C123456",  # This should be ignored
                        dm_channel_id="D67890",
                        response_url="https://hooks.slack.com/response",
                        user_name="testuser",
                    )

                    # Verify target_channel_id was used
                    mock_resolve.assert_called_once_with("C123456")
                    mock_validate.assert_called_once()
                    validate_call_args = mock_validate.call_args
                    assert "C123456" in str(validate_call_args)
