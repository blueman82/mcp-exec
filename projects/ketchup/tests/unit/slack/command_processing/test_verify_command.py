...
class TestSendValidationError:
    async def test_success(self) -> None:
        ...
        handler = AsyncMock()
        await verify_command.send_validation_error("U1", "C1", "err", handler, response_url="url")
...
class TestVerifyAndExtractCommand:
    ...
    async def test_valid_command(self, mock_extract: MagicMock, mock_context: MagicMock) -> None:
    ...
    @patch("packages.slack.command_processing.verify_command.extract_command_params")
    async def test_validation_error(self, mock_extract: MagicMock, mock_context: MagicMock) -> None:
    ...
    @patch("packages.slack.command_processing.verify_command.extract_command_params")
    async def test_unknown_error(self, mock_extract: MagicMock, mock_context: MagicMock) -> None:
