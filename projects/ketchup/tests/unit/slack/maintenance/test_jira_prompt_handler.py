...
def mock_maintenance_checker():
    ...
    checker.normalize_instance_name = MagicMock(
        side_effect=lambda x: x.replace("https://", "").replace(
            ".campaign.adobe.com", ""
        ).replace("-", "_")
    )
...
@pytest.mark.asyncio
async def test_post_jira_prompt_first_attempt(
    jira_prompt_handler, mock_posting_handler
):
    """Test posting JIRA prompt on first attempt."""
    mock_posting_handler.post_message.return_value = {
        "ok": True,
        "ts": "123.456"
    }
...
@pytest.mark.asyncio
async def test_post_jira_prompt_second_attempt(
    jira_prompt_handler, mock_posting_handler
):
...
@pytest.mark.asyncio
async def test_post_jira_prompt_failure(
    jira_prompt_handler, mock_posting_handler
):
...
@pytest.mark.asyncio
async def test_post_jira_prompt_exception(
    jira_prompt_handler, mock_posting_handler
):
...
@pytest.mark.asyncio
async def test_delete_prompt_message_success(
    jira_prompt_handler, mock_posting_handler
):
...
@pytest.mark.asyncio
async def test_delete_prompt_message_exception(
    jira_prompt_handler, mock_posting_handler
):
...
@pytest.mark.asyncio
async def test_check_recent_messages_for_jira_found(
    jira_prompt_handler, mock_channel_msg_ops
):
    ...
    mock_channel_msg_ops._make_api_request.return_value = {
        "messages": [
            {"ts": "123.456", "user": "U123456", "text": "@ketchup CPGNREQ-12345"},  # TEST_TICKET_ID
...
@pytest.mark.asyncio
async def test_check_recent_messages_no_mentions(
    jira_prompt_handler, mock_channel_msg_ops
):
...
@pytest.mark.asyncio
async def test_check_recent_messages_exception(
    jira_prompt_handler, mock_channel_msg_ops
):
...
async def test_wait_for_jira_reply_found(
    mock_sleep, jira_prompt_handler
):
    """Test waiting for JIRA reply and finding it."""
    # Mock DynamoDB methods to simulate app_mention event storing a reply
    with patch.object(
        jira_prompt_handler.db_store,
        "put_maintenance_prompt",
...
@pytest.mark.asyncio
@patch("asyncio.sleep", new_callable=AsyncMock)
async def test_wait_for_jira_reply_timeout(
    mock_sleep, jira_prompt_handler
):
...
@pytest.mark.asyncio
async def test_fetch_jira_ticket_success(
    jira_prompt_handler, mock_mcp_client
):
...
@pytest.mark.asyncio
async def test_fetch_jira_ticket_exception(
    jira_prompt_handler, mock_mcp_client
):
...
@pytest.mark.asyncio
async def test_post_no_maintenance_message_exception(
    jira_prompt_handler, mock_posting_handler
):
...
@pytest.mark.asyncio
async def test_post_timeout_message(
    jira_prompt_handler, mock_posting_handler
):
...
async def test_process_maintenance_check_found(
    ...
    mock_mcp_client.search_issues.return_value = {
        "issues": [{
            "fields": {
                "customfield_22302": "https://test-mkt-prod1.campaign.adobe.com"
            }
        }]
    }
    ...
    with patch.object(
        jira_prompt_handler,
        "_post_maintenance_found_message",
        new_callable=AsyncMock
    ) as mock_post_found:
        await jira_prompt_handler._process_maintenance_check(
            "C123", "CPGNREQ-12345"
        )
...
@pytest.mark.asyncio
async def test_process_maintenance_check_no_instance_url(
    jira_prompt_handler, mock_mcp_client
):
...
@pytest.mark.asyncio
async def test_process_maintenance_check_jira_fetch_failure(
    jira_prompt_handler, mock_mcp_client
):
...
@pytest.mark.asyncio
@patch("asyncio.sleep", new_callable=AsyncMock)
async def test_workflow_timeout_all_attempts(
    mock_sleep, jira_prompt_handler, mock_posting_handler
):
