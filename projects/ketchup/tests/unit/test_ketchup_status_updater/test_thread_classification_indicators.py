...
from packages.core.typed_di.service_registrations.protocols.slack_protocols import ChannelInfoOpsProtocol
...
async def test_integration_flow_with_thread_classification():
    ...
    with patch(
        ...
        with patch.object(generator, "_format_final_message") as mock_format:
            # Mock other required methods
            mock_deps["channel_operations"].get_channel_details.return_value = {
                "jira_ticket": ""
            }
            ...
            with patch.object(generator, "_verify_real_activity", return_value=True):
                with patch.object(generator, "_generate_ai_response", return_value="Status content"):
                    with patch.object(
                        generator, "_store_post_timestamp", return_value=None
                    ):
                        with patch.object(
                            generator, "check_for_activity",
