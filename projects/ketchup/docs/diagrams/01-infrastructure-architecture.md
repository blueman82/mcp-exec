...
This diagram shows the complete AWS infrastructure for the Ketchup Slack application using a C4 Container diagram style. The system uses a dual-server architecture with an Application Load Balancer distributing traffic. **prod1** runs all 7 containers including 2 singleton services (highlighted in red), while **prod2** runs only the 5 core containers for load distribution without duplicating scheduled jobs.
...
graph TB
    ...
    subgraph prod1["🖥️ prod1: ketchup-prod1.campaign.adobe.com"]
        ...
        subgraph Singletons1["🔴 Singleton Services (prod1 ONLY)"]
            unified["ketchup-unified-scheduler<br/>(5 Tasks: Status, Metadata,<br/>JIRA, Maintenance, PAT)"]
            csopm["ketchup-csopm-notifier<br/>(CSOPM Notifications<br/>08:00 & 16:00 UTC)"]
        end
    end
    ...
    unified -.->|"Read/Write"| DDB
    unified -->|"Post Updates"| SlackAPI
    unified <-.->|"JIRA API"| mcp1

    csopm -.->|"Read/Write"| DDB
    csopm -->|"Send DMs"| SlackAPI
    csopm <-.->|"JIRA API"| mcp1

    access1 -.->|"Poll Messages"| SQS
    access2 -.->|"Poll Messages"| SQS

    ECR -.->|"Pull Images"| prod1
    ECR -.->|"Pull Images"| prod2
    ...
    class unified,csopm,access1,access2 background
    class DDB,Secrets,SQS,ECR aws
    class unified,csopm singleton
...
**Singleton Pattern:**
- 2 services run ONLY on prod1 to prevent duplicate scheduled operations:
  - `ketchup-unified-scheduler` - Orchestrates 5 background tasks
  - `ketchup-csopm-notifier` - CSOPM assignment notifications
- These services are explicitly stopped/removed on prod2 during deployment
- Prevents race conditions and duplicate Slack posts/notifications

**Container Count:**
- **prod1:** 7 containers (5 core + 2 singletons)
- **prod2:** 5 containers (5 core only)
- **Total:** 12 containers across 2 servers
...
The `ketchup-unified-scheduler` container runs 5 internal tasks:
1. `status_updater` - Hourly AI-powered channel summaries (every 55 min)
2. `metadata_updater` - Channel metadata sync (every 15 min)
3. `jira_reporter` - Automated JIRA ticket creation (continuous)
4. `maintenance_fetcher` - Raven maintenance alerts (daily 1:30 UTC)
5. `pat_rotator` - JIRA PAT rotation (every 24 hours)
