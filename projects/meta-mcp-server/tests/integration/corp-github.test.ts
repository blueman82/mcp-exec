...
function hasCorpGithubConfig(): boolean {
...
const skipTests = !isDockerAvailable() || !hasCorpGithubConfig();
...
describe('Corp-Github Docker Server', () => {
  let pool: ServerPool;
  ...
  describe('spawn configuration', () => {
    ...
    it.skipIf(skipTests)('includes corp-github docker image in args', () => {
      const config = getServerConfig(SERVER_ID);
      expect(config?.args).toBeDefined();

      const imageArg = config!.args!.find((arg) => arg.includes('/') || arg.includes(':'));
      expect(imageArg).toBeDefined();
      expect(imageArg).toContain('github');
    });
  });

  describe('server listing', () => {
    it.skipIf(skipTests)('list_servers includes corp-github', async () => {
  ...
  describe('tool discovery', () => {
    it.skipIf(skipTests || !hasAvailableImage())(
      'get_server_tools returns github tools',
      async () => {
        const result = await callToolHandler({
  ...
  describe('tool execution', () => {
    it.skipIf(skipTests || !hasAvailableImage())(
      'can call a tool on corp-github server',
      async () => {
        ...
        if (toolsData.tools.length === 0) {
          console.warn('No tools available on corp-github server');
          return;
        }

        // Find a tool that doesn't require arguments (has no required fields)
        const noArgTool = toolsData.tools.find((t: { inputSchema?: { required?: string[] } }) => 
        ...
        if (!noArgTool) {
          ...
          console.warn('All corp-github tools require arguments - skipping execution test');
          return;
        }
