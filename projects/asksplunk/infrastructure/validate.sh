#!/usr/bin/env bash
#
# validate.sh - Local CI validation for AskSplunk
#
# This script runs all validation checks (pytest, ruff, black) before
...
SRC_DIR="${PROJECT_ROOT}/src/asksplunk"
...
show_help() {
    cat << EOF
${BOLD}AskSplunk Local CI Validation${NC}
...
check_black() {
    ...
    if [ "$FIX" = true ]; then
        ...
        if black "$SRC_DIR" "$TESTS_DIR" --check --quiet; then
            ...
            log_error "Code formatting issues found. Run './validate.sh --fix' to fix"
...
check_pytest() {
    ...
    if [ "$VERBOSE" = true ]; then
        pytest "$TESTS_DIR/unit" --cov="$SRC_DIR" --cov-report=term-missing -v
    else
        pytest "$TESTS_DIR/unit" --cov="$SRC_DIR" --cov-report=term --quiet
    fi
...
log_section "AskSplunk Local CI Validation"
...
if [ $CHECKS_FAILED -eq 0 ]; then
    ...
    echo ""
    log_error "Try './validate.sh --fix' to auto-fix style issues."
    exit 1
fi
