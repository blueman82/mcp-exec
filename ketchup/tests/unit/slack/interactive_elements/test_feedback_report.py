...
def handler(mocker):
    ...
    mock_posting = mocker.patch(
        "packages.slack.messages.posting.SlackPostingHandler", spec=True
    )
...
async def test_open_feedback_report_modal_slack_error(handler, mocker) -> None:
    ...
    mock_response = MockAiohttpResponse(
        status=200, json_data={"ok": False, "error": "test_error"}
    )
...
async def test_open_feedback_report_modal_exception(handler, mocker) -> None:
    ...
    with patch(
        "aiohttp.ClientSession", side_effect=aiohttp.ClientError("Network failed")
    ):
        result = await handler.open_feedback_report_modal("TRIGGER123")
...
async def test_send_feedback_report_to_channel_success(handler, monkeypatch) -> None:
    ...
    result = await handler.send_feedback_report_to_channel(
        "U1", "title", "desc", "TRIGGER123"
    )
    assert result is True
