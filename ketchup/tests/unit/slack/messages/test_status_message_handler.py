...
"""
Unit tests for packages.slack.blockkits.handlers.status.StatusMessageHandler
...
@pytest.fixture
def handler():
    """Fixture for a configured StatusMessageHandler with all dependencies mocked."""
    h = StatusMessageHandler()
...
async def test_send_message_normal_with_feedback(handler):
    ...
    await handler.send_message(
        combined_command="/status foo",
        response_url="http://slack.com/response",
        response_text="Status content",
        query="foo",
        target_channel="C1",
    )
    ...
    assert "Status content" in call["message"]
...
async def test_send_message_posting_error_triggers_fallback(handler):
    ...
    assert "Status content" in fallback_call["message"]
