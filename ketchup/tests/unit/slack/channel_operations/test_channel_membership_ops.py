...
async def test_fetch_channel_page_success(
    ...
    with patch(
        "packages.slack.channel_operations.channel_membership_ops.logger"
    ) as mock_logger:
...
async def test_fetch_all_channel_memberships_pagination(
    ...
    monkeypatch.setattr(
        ops, "_fetch_channel_page", AsyncMock(side_effect=fake_fetch_page)
    )
...
async def test_lookup_membership_of_channels_delegates(
    ...
    monkeypatch.setattr(
        ops._backoff_strategy, "execute", AsyncMock(return_value=[{"id": "C1"}])
    )
    with patch(
        "packages.slack.channel_operations.channel_membership_ops.logger"
    ) as mock_logger:
