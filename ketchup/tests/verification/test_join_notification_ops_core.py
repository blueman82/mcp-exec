...
class TestJoinNotificationOpsCore:
    ...
    @pytest.mark.asyncio
    async def test_successful_notification_tracking(self, join_notification_ops, mock_dynamodb_client):
    ...
    @pytest.mark.asyncio
    async def test_disabled_notification_tracking(self, join_notification_ops, mock_dynamodb_client):
...
class TestJoinNotificationRegression:
    ...
    def test_failure_reason_enum_values(self):
        ...
        expected_values = {
            "slack_rate_limited", "not_in_channel", "permission_denied",
            "slack_api_error", "ai_generation_failed", "ai_timeout",
            "network_error", "internal_error", "data_collection_failed",
        }
