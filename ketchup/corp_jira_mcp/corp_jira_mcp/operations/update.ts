...
export async function updateJiraIssue(params: UpdateJiraIssueRequest): Promise<{ success: boolean; message: string; data?: unknown }> {
  ...
  try {
    // Ensure the Jira issue key is present in the labels
    let fieldsWithoutStatus = fields ? { ...fields } : {};
    if (fieldsWithoutStatus.labels) {
      if (!fieldsWithoutStatus.labels.includes(issueIdOrKey)) {
        fieldsWithoutStatus.labels = [...fieldsWithoutStatus.labels, issueIdOrKey];
      }
    } else {
      fieldsWithoutStatus.labels = [issueIdOrKey];
    }
