...
This module contains the AutoStatusProcessor class, which is responsible for processing all channels eligible for auto-status.
"""
import os
import asyncio
...
from packages.core.constants import TEST_CHANNEL, FEEDBACK_CHANNEL
...
class AutoStatusProcessor:
    ...
    async def process_all_channels(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if hasattr(self.db_store, 'client'):
                    response = await self.db_store.client.get_item(
                        key={
                            "PK": {"S": "SYSTEM_SETTINGS"},
                            "SK": {"S": "AUTO_STATUS_CONFIG"}
                        },
                        table_name=self.db_store.table_name
                    )
                    # Check if item exists and if paused flag is true
                    if response.get("Item") and response["Item"].get("paused", {}).get("BOOL", False):
                    ...
                    try:
                        pause_settings = await self.db_store.get_item("SYSTEM_SETTINGS", "AUTO_STATUS_CONFIG")
            ...
            if TEST_CHANNEL_ID:
                ...
                test_channel = next(
                    (ch for ch in all_channels if ch.get("channel_id") == TEST_CHANNEL_ID), 
                    None
                )
                if test_channel:
                ...
                logger.info(f"Global mode: Processing all {len(channels_to_process)} active channels")
            else:
                # Check each channel for feature flag
                if self.feature_service:
                    for channel in all_channels:
                        channel_id = channel.get("channel_id")
                        if await self.feature_service.is_status_updater_enabled_for_channel(channel_id):
                            channels_to_process.append(channel)
                            
                    logger.info(f"Processing {len(channels_to_process)} enabled channels")
                else:
                    logger.warning("Feature service not available, falling back to TEST_CHANNEL only")
                ...
                if not channels_to_process:
                    ...
                    test_channel = next(
                        (ch for ch in all_channels if ch.get("channel_id") == TEST_CHANNEL), 
                        None
                    )
                    if test_channel:
                        channels_to_process = [test_channel]
            ...
            for channel in channels_to_process:
                ...
                try:
                    ...
                    should_process = (FeatureFlags.is_status_updater_global() or 
                                    await self._should_process_channel(channel))
                    
                    if should_process:
                        if FeatureFlags.is_status_updater_global():
                            logger.info(f"Global mode: Processing channel {channel_id} (ignoring timing)")
    ...
    async def _should_process_channel(self, channel: Dict[str, Any]) -> bool:
        ...
        if last_run == 0:
            logger.info(f"Channel {channel.get('channel_id')} has never been updated, processing now")
            return True
        ...
        if should_process:
            logger.info(f"Channel {channel.get('channel_id')} is due for update (last run: {last_run_time.strftime('%H:%M:%S')})")
    ...
    async def _process_channel(self, channel: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if success:
                ...
                await self.channel_operations.update_channel_fields(
                    channel_id=channel_id,
                    updates={"auto_status_attempt_count": attempt_count + 1}
                )
                return False
