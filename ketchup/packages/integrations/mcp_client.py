...
class MCPClient:
    ...
    async def create_pat(
        ...
        try:
            ...
            if result.get("success") and "data" in result:
                ...
                logger.error(f"Failed to create PAT: {error_msg}")
    ...
    async def validate_pat(self, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error validating PAT: {e}")
            raise
    ...
    async def revoke_pat(self, token_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.get("success"):
                logger.info(f"PAT revoked successfully: {token_id}")
            else:
                logger.warning(
                    f"Failed to revoke PAT {token_id}: {result.get('message', 'Unknown error')}"
                )

            return result
        except Exception as e:
            logger.error(f"Error revoking PAT {token_id}: {e}")
            raise
