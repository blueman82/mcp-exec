...
def _register_utility_services(manager: "ServiceRegistrationManager") -> None:
    ...
    try:
        ...
        async def create_join_notification_ops(resolver) -> JoinNotificationOps:
            ...
            table_name = dynamodb_config.table_name if hasattr(dynamodb_config, 'table_name') else "ketchup_channel_information"

            return JoinNotificationOps(
                client=dynamodb_async_client,
...
def _register_access_request_services(manager: "ServiceRegistrationManager") -> None:
    ...
    if not ACCESS_REQUEST_IMPORTS_AVAILABLE:
        logger.warning("AccessRequest services not available - registering placeholder implementations")
