...
from packages.slack.services.metrics_data_collector import (
...
class MetricsExportHandler:
    ...
    async def handle_metrics_request(
        ...
        try:
            ...
            if success:
                completion_msg = (
                    "✅ Metrics dashboard generated and sent via DM!"
                )
                if response_url:
    ...
    async def _get_upload_url(
        ...
        url = (
            f"{self._slack_posting_handler.config.get_api_base_url()}"
            "/files.getUploadURLExternal"
        )
        headers = {
            "Authorization": (
                f"Bearer {self._slack_posting_handler._slack_token}"
            ),
        ...
        if not upload_url_response.get("ok"):
            logger.error(
                f"Failed to get upload URL: "
                f"{upload_url_response.get('error', 'Unknown error')}"
            )
            return None, None
    ...
    async def _complete_upload(
        ...
        complete_payload = {
            ...
            "initial_comment": (
                "Here's your Ketchup metrics dashboard! 📊"
            ),
        }

        complete_response = await (
            self._slack_posting_handler.api_call(
        ...
        if complete_response.get("ok"):
            ...
            logger.error(
                ...
                f"{complete_response.get('error', 'Unknown error')}"
            )
            return False
    ...
    async def _upload_html_to_slack(
        ...
        try:
            ...
            return await self._complete_upload(
                file_id, filename, target_channel
            )
