...
# Install SSSD packages (matching asksplunk-prod)
...
# SSSD configuration will be deployed manually from asksplunk-prod
log_message "SSSD installation complete - manual configuration required via post-launch setup"
...
# Configure SSH to use SSSD for public keys (matching asksplunk-prod)
log_message "Configuring SSH to retrieve keys from LDAP..."
...
SSHEOF
    ...
    log_message "SSH configured for LDAP public keys"
fi
