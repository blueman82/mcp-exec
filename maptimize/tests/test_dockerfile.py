...
def test_dockerfile_multi_stage():
    ...
    assert from_count >= 2, f"Dockerfile should use multi-stage build with at least 2 FROM statements, found {from_count}"
...
def test_dockerfile_non_root_user():
    ...
    assert 'root' not in final_user or '1000' in final_user, "Dockerfile should use non-root user (UID 1000)"
